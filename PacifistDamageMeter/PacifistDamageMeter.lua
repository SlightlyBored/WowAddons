...
function AutoActivate(active)
    if PacifistDamageMeterGlobalData.UserEnabled == true then
        Activate(active)
    else
        if Loaded >= 2 then
...
function UserActivate(active)
    if active ~= nil then
        PacifistDamageMeterGlobalData.UserEnabled = active
    else
        PacifistDamageMeterGlobalData.UserEnabled = not PacifistDamageMeterGlobalData.UserEnabled
    end

    Activate(PacifistDamageMeterGlobalData.UserEnabled == true)
end
...
local function EventHandler(self, event, ...)
    ...
    elseif event == "ADDON_LOADED" then
        if (...) == "PacifistDamageMeter" then
            PacifistDamageMeterGlobalData = PacifistDamageMeterGlobalData or {}

            if PacifistDamageMeterGlobalData.UserEnabled == nil then
                PacifistDamageMeterGlobalData.UserEnabled = true
            end
            PacifistDamageMeterCharacterData = PacifistDamageMeterCharacterData or {}
            Loaded = Loaded + 1
        end
    end
end
